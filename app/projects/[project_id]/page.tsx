...
export default function ProjectDetailPage({ params }: { params: Promise<{ project_id: string }> }) {
  ...
  // Realtime Presence for Active status
  useEffect(() => {
    if (!currentUserId) return;

    const channel = supabase.channel(`project_${project_id}_presence`, {
      config: {
        presence: {
          key: currentUserId,
        },
  ...
  }, [project_id, currentUserId, supabase]);
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white p-8 rounded-lg border border-slate-200">
            ...
            <table className="w-full text-left">
              <thead><tr className="text-[11px] text-slate-400 uppercase border-b border-slate-100"><th className="px-4 py-3">メンバー</th><th className="px-4 py-3">役割</th><th className="px-4 py-3">担当課題数</th><th className="px-4 py-3 text-center">ステータス</th></tr></thead>
              <tbody className="divide-y divide-slate-50">{stats.memberStats.map(m => {
                const isActive = onlineUsers.has(m.user_id) || m.user_id === currentUserId;
                return (
                <tr key={m.user_id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-4 flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium">{m.users?.user_name}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${m.role === 'leader' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>
                  ...
                  <td className="px-4 py-4 text-center">
                    {isActive ? (
                      <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-green-50 text-green-700 text-[10px] font-bold border border-green-100 animate-pulse">
                        <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span>
                        ACTIVE
                      </span>
                    ) : (
                      <span className="text-[10px] font-medium text-slate-300">OFFLINE</span>
                    )}
                  </td>
                </tr>
                );
